...
const keyArbitrary = fc.string({ minLength: 1, maxLength: 6 }).filter((s) => !s.includes('.') && !s.startsWith('$'))

const intArbitrary = fc.integer({ min: -1_000_000, max: 1_000_000 })
const finiteNumberArbitrary = fc.double({ noNaN: true, min: -1e9, max: 1e9 })

const leafArbitrary = fc.oneof(intArbitrary, fc.string(), fc.boolean(), fc.constant(null))

const docArbitrary: fc.Arbitrary<Record<string, unknown>> = fc.dictionary(keyArbitrary, leafArbitrary, { maxKeys: 5 })

const intArrayArbitrary = fc.array(intArbitrary, { maxLength: 8 })
...
describe('applyUpdate — $set / $unset properties', () => {
  it('$set on a top-level key makes that key readable with the new value', () => {
    fc.assert(
      fc.property(docArbitrary, keyArbitrary, leafArbitrary, (doc, key, value) => {
  ...
  it('$unset followed by absence: after $unset the key is not an own-prop of the result', () => {
    fc.assert(
      fc.property(docArbitrary, keyArbitrary, leafArbitrary, (doc, key, seedValue) => {
  ...
  it('$set then $set (same key) is equivalent to a single $set with the final value', () => {
    fc.assert(
      fc.property(docArbitrary, keyArbitrary, leafArbitrary, leafArbitrary, (doc, key, v1, v2) => {
...
describe('applyUpdate — $inc / $mul properties', () => {
  it('$inc is additive: inc(n) ∘ inc(m) === inc(n + m)', () => {
    fc.assert(
      fc.property(keyArbitrary, finiteNumberArbitrary, intArbitrary, intArbitrary, (key, seed, n, m) => {
        const base = { [key]: seed }
  ...
  it('$inc on a missing field treats the original as 0', () => {
    fc.assert(
      fc.property(keyArbitrary, intArbitrary, (key, delta) => {
  ...
  it('$mul with factor 1 is identity on numeric fields', () => {
    fc.assert(
      fc.property(keyArbitrary, finiteNumberArbitrary, (key, seed) => {
        const base = { [key]: seed }
...
describe('applyUpdate — array operator properties', () => {
  it('$push grows the array by exactly 1 (single value)', () => {
    fc.assert(
      fc.property(keyArbitrary, intArrayArbitrary, intArbitrary, (key, arr, value) => {
  ...
  it('$push with $each grows by exactly the length of $each', () => {
    fc.assert(
      fc.property(keyArbitrary, intArrayArbitrary, intArrayArbitrary, (key, arr, extras) => {
  ...
  it('$pullAll removes every element of the target set', () => {
    fc.assert(
      fc.property(keyArbitrary, intArrayArbitrary, intArrayArbitrary, (key, arr, targets) => {
  ...
  it('$pop with direction 1 shrinks by 1 (non-empty) and preserves every non-last element', () => {
    fc.assert(
      fc.property(
        keyArbitrary,
        intArrayArbitrary.filter((a) => a.length > 0),
        (key, arr) => {
...
describe('applyUpdate — $min / $max properties', () => {
  it('$min never produces a value greater than min(seed, candidate)', () => {
    fc.assert(
      fc.property(keyArbitrary, finiteNumberArbitrary, finiteNumberArbitrary, (key, seed, candidate) => {
